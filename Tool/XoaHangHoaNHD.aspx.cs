...
public partial class Tool_XoaHangHoaNHD : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string dieukien = "and sp.trangthai = 'NHD'";
        string dieukien1 = string.Format("and sp.ngayngunghd between convert(datetime, '{0}', 103) and convert(datetime, '{1}', 103)", startdate, enddate);
        if (typepost == "Huy")
        {
            dieukien = "";
            dieukien1 = "";
        ...
        else if (typepost == "Huy")
        {
            Response.Write("Phải chọn tập tin và tập tin phải có nội dung.");
            arrID = "and sp.ma_sanpham in ('Error')";
        }
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (typepost == "Huy")
                            {
                                ...
                                db.SubmitChanges();
                                msgDT += string.Format("<div style='color: blue'>{0} đã chuyển sang ngưng hoạt động.</div>", sp.ma_sanpham);
                            }
                            #endregion
                            ...
                            if (typepost == "Xoa")
                            {
                                ...
                                try
                                {
                                    foreach (md_giasanpham gsp in db.md_giasanphams.Where(s => s.md_sanpham_id == sp.md_sanpham_id))
                                    {
                                        db.md_giasanphams.DeleteOnSubmit(gsp);
                                    }
                                    db.SubmitChanges();
                                    msgDT += string.Format("<div style='color: blue'>{0} đã xóa giá.</div>", sp.ma_sanpham);
                                }
                                catch (Exception ex1)
                                {
                                    msgDT += string.Format("<div style='color: red'>{0} xóa giá lỗi {1}.</div>", sp.ma_sanpham, ex1.Message);
                                }
                                #endregion

                                #region Xóa Hàng hóa độc quyền
                                try
                                {
                                    foreach (md_hanghoadocquyen hhdq in db.md_hanghoadocquyens.Where(s => s.md_sanpham_id == sp.md_sanpham_id))
                                    {
                                        db.md_hanghoadocquyens.DeleteOnSubmit(hhdq);
                                    }
                                    db.SubmitChanges();
                                    msgDT += string.Format("<div style='color: blue'>{0} đã hàng hóa độc quyền.</div>", sp.ma_sanpham);
                                }
                                catch (Exception ex1)
                                {
                                    msgDT += string.Format("<div style='color: red'>{0} xóa hàng hóa độc quyền lỗi {1}.</div>", sp.ma_sanpham, ex1.Message);
                                }
                                #endregion
