<%@ Page Language="C#" AutoEventWireup="true" CodeFile="inc-doitacketoan.aspx.cs" Inherits="inc_doitacketoan" %>
<%@ Register src="jqGrid.ascx" tagname="jqGrid" tagprefix="uc1" %>
        ...
        $(function() {
            $('#lay-center-dtkd').parent().layout({
                ...
                , onresize_end: function() {
                    ...
                    $("#grid_ds_doitackinhdoanhkt").setGridHeight(o - 90);
                }
                },
                center: {
                    onresize_end: function() {
                        var o = $("#lay-center-dtkd").height();
                        $("#grid_ds_nguoilienhekt").setGridHeight(o - 70);
                    }
                }
            });
        });
                ...
                click: function() {
                    ...
                    if (typeof printType != 'undefined') {
                        if (printType == 'printdtkd') {
							...
							window.open(printURL + "?c_donhang_id=" + poId + "&where_ex=and ldt.ma_loaidtkd = 'NCCVT'", "In Chiết Xuất Thông Tin Đối Tác Kinh Doanh", windowSize);
                        }
                    } else {
	...
	function loading() {
		$('#DelTbl_grid_ds_doitackinhdoanhkt').prepend('<div style="display:block" id="wait"><img style="width:30px; height:30px" src="iconcollection/loading.gif"/></div></h3>');
	}
    ...
    <uc1:jqGrid  ID="grid_ds_doitackinhdoanhkt" 
            SortName="ma_dtkd" 
            UrlFileAction="Controllers/PartnerKeToanController.ashx" 
            ...
            OndblClickRow = "
				function(rowid) { 
					// $(this).jqGrid('editGridRow', rowid, {
                        ...
                        // }
                    // });
					$('#edit_grid_ds_doitackinhdoanhkt').click();
				}
			"
            OnSelectRow = "function(ids) {
		                if(ids == null) {
			                ids=0;
			                if($('#grid_ds_nguoilienhekt').jqGrid('getGridParam','records') &gt; 0 )
			                {
				                $('#grid_ds_nguoilienhekt').jqGrid('setGridParam',{url:'Controllers/NguoiLienHeController.ashx?&md_doitackinhdoanh_id='+ids,page:1});
				                $('#grid_ds_nguoilienhekt').jqGrid().trigger('reloadGrid');
				            } 
				        } else {
			                $('#grid_ds_nguoilienhekt').jqGrid('setGridParam',{url:'Controllers/NguoiLienHeController.ashx?&md_doitackinhdoanh_id='+ids,page:1});
			                $('#grid_ds_nguoilienhekt').jqGrid().trigger('reloadGrid');			
            ...
            },
            {
                 ...
                 , width: 100
                 , edittype: 'select'
                 , editoptions: { dataUrl: 'Controllers/PartnerKeToanController.ashx?action=getop_loaidtkd' }
...
<DIV class="ui-layout-center ui-widget-content" id="lay-center-dtkd" style="padding:0;">
    ...
    <uc1:jqGrid  ID="grid_ds_nguoilienhekt" 
            SortName="cl.ten" 
            ...
            AddFormOptions=" 
                beforeShowForm: function (formid) {
                        var masterId = $('#grid_ds_doitackinhdoanhkt').jqGrid('getGridParam', 'selrow');
...
<script type="text/javascript">
    createRightPanel('grid_ds_doitackinhdoanhkt');
    createRightPanel('grid_ds_nguoilienhekt');
</script>
