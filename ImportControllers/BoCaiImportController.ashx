<%@ WebHandler Language="C#" Class="BoCaiImportController" %>
...
public class BoCaiImportController : IHttpHandler
{
    ...
    public void NewFromCellCollection(HttpContext context, CellCollection cellCollection)
    {
        for (int i = 1; i < cellCollection.Rows.Count; i++)
        {
            try
            {
                ...
                md_bocai bocai = ImportUtils.getBoCai(cellMa);

                if (bocai == null)
                {
                    BoCaiImport import = new BoCaiImport(cellMa, cellTVN, cellTAN, cellTVD, cellTAD);
                    import.Import();
                    msg += String.Format("<div style=\"color:green\">Đã tạo một bộ cái mới!</div>");
                }
                else
                {
                    BoCaiImport import = new BoCaiImport(cellMa, cellTVN, cellTAN, cellTVD, cellTAD);
                    import.Update();
                    msg += String.Format("<div style=\"color:green\">Cập nhật bộ cái thành công!</div>");
                }
