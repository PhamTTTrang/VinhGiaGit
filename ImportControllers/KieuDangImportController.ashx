<%@ WebHandler Language="C#" Class="KieuDangImportController" %>
...
public class KieuDangImportController : IHttpHandler
{
    ...
    public void NewFromCellCollection(HttpContext context, CellCollection cellCollection)
    {
        for (int i = 1; i < cellCollection.Rows.Count; i++)
        {
            try
            {
                ...
                String cellMaKD = row.GetCell(0).Value == null ? String.Empty : row.GetCell(0).Value.ToString();
                ...
                if (kd == null)
                {
                    KieuDangImport import = new KieuDangImport(cellMaKD, cellTVN, cellTAN, cellGC);
                    import.Import();

                    msg += String.Format("<div style=\"color:green\">Đã tạo một kiểu dáng sản phẩm mới!</div>");
                }
				else if(kd != null)
				{
                    ...
                    msg += String.Format("<div style=\"color:green\">Cập nhật kiểu dáng sản phẩm thành công!</div>");
                }
