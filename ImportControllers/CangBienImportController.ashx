<%@ WebHandler Language="C#" Class="CangBienImportController" %>
...
public class CangBienImportController : IHttpHandler {
    ...
    public void NewFromCellCollection(HttpContext context, CellCollection cellCollection)
    {
        for (int i = 1; i < cellCollection.Rows.Count; i++)
        {
            try
            {
                ...
                md_cangbien cangbien = ImportUtils.getCangBien(cellMa);
                if (cangbien == null)
                {
                    CangBienImport import = new CangBienImport(cellMa, cellTen);
                    import.Import();
                    msg += String.Format("<div style=\"color:green\">Đã tạo một cảng biển mới!</div>");
                }
                else 
                {
                    CangBienImport import = new CangBienImport(cellMa, cellTen);
                    import.Update();
                    msg += String.Format("<div style=\"color:green\">Cập nhật cảng biển thành công!</div>");
                }
