<%@ Page Language="C#" AutoEventWireup="true" CodeFile="inc-hanghoa-nhd.aspx.cs" Inherits="inc_hanghoa_nhd" %>
<%@ Register src="jqGrid.ascx" tagname="jqGrid" tagprefix="uc1" %>
<script>
    $(function() {
		setTimeout(function () {
			$('#grid_ds_sanpham_nhd-pager_left table tr').append(
				'<td class="ui-pg-button ui-corner-all" title="Chuyển trạng thái nhanh" id="ctt_grid_ds_sanpham_nhd">' +
				'<div class="ui-pg-div"><span onclick="ChangeStatusALL_NHD()" class="ui-icon ui-icon-circle-plus"></span></div><td>')
			, 5000
		});

		$('#lay-center-sanpham_nhd').parent().layout({
            south: {
                size: "40%"
                    , minSize: "0%"
                    , maxSize: "100%"
                    , onresize_end: function() {
                        var h = $("#lay-south-sanpham_nhd").height();

                        // sett height
                        $("#grid_ds_quycachdonggoi_nhd").setGridHeight(h + 4 - 80);
                        $("#grid_ds_giafob_nhd").setGridHeight(h + 4 - 80);
                        $("#grid_ds_khachhangdocquyen_sp_nhd").setGridHeight(h + 4 - 80);
                        $("#grid_ds_cangxuathang_nhd").setGridHeight(h + 4 - 80);
                        $("#grid_ds_bientau_nhd").setGridHeight(h + 4 - 80);
                        $("#grid_ds_gianoi_nhd").setGridHeight(h + 4 - 80);
						$("#grid_ds_nhacungung_nhd").setGridHeight(h + 4 - 80);
                    }
            }
            , center: {
                onresize_end: function() {
                var h = $("#lay-center-sanpham_nhd").height();
                var w = $("#lay-center-sanpham_nhd").width();
                $("#grid_ds_sanpham_nhd").setGridHeight(h + 4 - 70);
                }
            }
        });
        $('#lay-south-sanpham_nhd').layout({
            center: {
                onresize_end: function() {
                var w = $("#tab-product-details_nhd").width();
                    //set width
                $("#grid_ds_quycachdonggoi_nhd").setGridWidth(w);
                $("#grid_ds_giafob_nhd").setGridWidth(w);
                $("#grid_ds_khachhangdocquyen_sp_nhd").setGridWidth(w);
                $("#grid_ds_cangxuathang_nhd").setGridWidth(w);
                $("#grid_ds_bientau_nhd").setGridWidth(w);
                $("#grid_ds_gianoi_nhd").setGridWidth(w); 
				$("#grid_ds_nhacungung_nhd").setGridWidth(w);
                }
            },
            east: {
                size: 200
        ...
        $('#tab-product-details_nhd').tabs();
    });
	
	function ChangeStatusALL_NHD() {
                ...
                , buttons: [
                {
                    ...
                    click: function () {
                        ...
                        if (typeof statusType != 'undefined') {
							...
							{ filters : $('#grid_ds_sanpham_nhd').getGridParam("postData").filters, trangthai_ht: 'NHD' },
							function (result) {
								$("#wait").css("display", "none");
                                $("#btn-close span").text("Thoát");
                                $("#btn-close").button("enable");
                                $("#dialog-changestatus").find("#caution").append(result);
                                $('#grid_ds_sanpham_nhd').jqGrid().trigger('reloadGrid');
                            });
                        } else {
    ...
    function ChangeStatus() {
        var md_sanpham_id = $('#grid_ds_sanpham_nhd').jqGrid('getGridParam', 'selrow');
	...
	function funcPrintSP() {
        funcPrintSPPublic('grid_ds_sanpham_nhd','NHD');
    }
</script>

<DIV class="ui-layout-center ui-widget-content" id="lay-center-sanpham_nhd">
    <uc1:jqGrid  ID="grid_ds_sanpham_nhd" 
            SortName="ma_sanpham"
            SortOrder ="asc"
            UrlFileAction="Controllers/ProductPauseController.ashx" 
            ...
            {
                fixed: true, name: 'ngayngunghd'
                , label: 'Ngày ngưng HĐ'
                , index: 'sp.ngayngunghd'
                , width: 50
                , editable:false
                , edittype: 'text'
                , hidden: false 
            ...
            OnSelectRow = "function(ids) {
		                    if(ids == null) {
			                    ...
			                    if($('#grid_ds_quycachdonggoi_nhd').jqGrid('getGridParam','records') &gt; 0 )
			                    {
				                    $('#grid_ds_quycachdonggoi_nhd').jqGrid('setGridParam',{url:'Controllers/QuyCachDongGoiController.ashx?&productId='+ids,page:1});
				                    $('#grid_ds_quycachdonggoi_nhd').jqGrid().trigger('reloadGrid');
				                }
				                
				                if($('#grid_ds_giafob_nhd').jqGrid('getGridParam','records') &gt; 0 )
			                    {
				                    $('#grid_ds_giafob_nhd').jqGrid('setGridParam',{url:'Controllers/PriceFOBController.ashx?&productId='+ids,page:1});
				                    $('#grid_ds_giafob_nhd').jqGrid().trigger('reloadGrid');
				                }
				                
				                if($('#grid_ds_gianoi_nhd').jqGrid('getGridParam','records') &gt; 0 )
			                    {
				                    $('#grid_ds_gianoi_nhd').jqGrid('setGridParam',{url:'Controllers/PriceSaleController.ashx?&productId='+ids,page:1});
				                    $('#grid_ds_gianoi_nhd').jqGrid().trigger('reloadGrid');
				                }
				                
				                if($('#grid_ds_khachhangdocquyen_sp_nhd').jqGrid('getGridParam','records') &gt; 0 )
			                    {
				                    $('#grid_ds_khachhangdocquyen_sp_nhd').jqGrid('setGridParam',{url:'Controllers/QuyDinhVoiKhachHangController.ashx?&productId='+ids,page:1});
				                    $('#grid_ds_khachhangdocquyen_sp_nhd').jqGrid().trigger('reloadGrid');
				                } 
				                
				                if($('#grid_ds_cangxuathang_nhd').jqGrid('getGridParam','records') &gt; 0 )
			                    {
				                    $('#grid_ds_cangxuathang_nhd').jqGrid('setGridParam',{url:'Controllers/CangXuatHangController.ashx?&productId='+ids,page:1});
				                    $('#grid_ds_cangxuathang_nhd').jqGrid().trigger('reloadGrid');
				                } 
				                
				                if($('#grid_ds_bientau_nhd').jqGrid('getGridParam','records') &gt; 0 )
			                    {
				                    $('#grid_ds_bientau_nhd').jqGrid('setGridParam',{url:'Controllers/BienTauController.ashx?&productId='+ids,page:1});
				                    $('#grid_ds_bientau_nhd').jqGrid().trigger('reloadGrid');
				                } 
								
								if($('#grid_ds_nhacungung_nhd').jqGrid('getGridParam','records') &gt; 0 )
			                    {
                                    $('#grid_ds_nhacungung_nhd').jqGrid('setGridParam',{url:'Controllers/NhaCungUngMacDinhController.ashx?&productId='+ids,page:1});
			                        $('#grid_ds_nhacungung_nhd').jqGrid().trigger('reloadGrid');
                                }
				            } 
				            else 
				            {
                                ...
                                $('#img-product_nhd').attr('src', '');
                                $('#img-product_nhd').attr('src', 'Controllers/API_System.ashx?oper=loadImageProduct&msp=' + ma_sanpham + '&ver=' + uuidv4());
		                    }

                            $('#grid_hanghoanhd_danhmuchanghoa').jqGrid('getGridParam', 'postData').id = ids;
                            $('#grid_hanghoanhd_danhmuchanghoa').jqGrid().trigger('reloadGrid');
		                }"
            Height = "420"
            
            runat="server" />
</DIV>
<DIV class="ui-layout-south" id="lay-south-sanpham_nhd"> 
    <div class="ui-layout-center" id="tab-product-details_nhd">
       <ul>
		    ...
		    <div id="nav-tabs-1">
		        <uc1:jqGrid  ID="grid_ds_quycachdonggoi_nhd" 
		    ...
		    <div id="nav-tabs-2">
		        <uc1:jqGrid  ID="grid_ds_giafob_nhd"
		    ...
		    <div id="nav-tabs-3">
		        <uc1:jqGrid  ID="grid_ds_gianoi_nhd"
                SortName="md_giasanpham_id" 
                UrlFileAction="Controllers/PriceFOBController.ashx" 
                ColNames="['md_giasanpham_id', 'Tên Phiên Bản','Tên Sản Phẩm', 'Giá'
                , 'Ðồng Tiền', 'Ngày Hiệu Lực', 'Ngày Tạo', 'Người Tạo', 'Ngày Cập Nhật', 'Người Cập Nhật', 'Mô tả', 'Hoạt động']"
		    ...
		    <div id="nav-tabs-4">
		        <uc1:jqGrid  ID="grid_ds_khachhangdocquyen_sp_nhd"
		    ...
		    <div id="nav-tabs-5">
		        <uc1:jqGrid  ID="grid_ds_cangxuathang_nhd"
                    ...
                    AddFormOptions="
                        ...
                        , beforeShowForm: function (formid) {
                            var masterId = $('#grid_ds_sanpham_nhd').jqGrid('getGridParam', 'selrow');
		    ...
		    <div id="nav-tabs-6">
		        <uc1:jqGrid  ID="grid_ds_bientau_nhd"
			...
			<div id="nav-tabs-7">
		        <uc1:jqGrid  ID="grid_ds_nhacungung_nhd"
		    ...
		    <div id="nav-tabs-8">
		        <uc1:jqGrid  ID="grid_hanghoanhd_danhmuchanghoa"
                    ...
                    AddFormOptions="
                        ...
                        , beforeShowForm: function (formid) {
                            let masterId = $('#grid_ds_sanpham_nhd').jqGrid('getGridParam', 'selrow');
                            if(masterId == null)
                            {
                                $(formid).parent().parent().parent().dialog('destroy').remove();
                                alert('Hãy chọn một sản phẩm mới có thể tạo chi tiết.!');
    ...
    <div class="ui-layout-east" style="text-align:center; background:#f4f0ec">
        <table style="width:100%; height:100%">
            <tr>
                <td>
                    <img id="img-product_nhd" class="loadingIMG" style="max-width:100%; margin: 0px auto; min-width: 40px;" src="Controllers/API_System.ashx?oper=loadImageProduct&msp=default" />
                </td>
            </tr>
        </table>
    </div>
...
<script>
	if(xemgiaHHNHD.toLowerCase() == 'false') {
		var parent = $('#tab-product-details_nhd');
	...
	if(chietxuatHHNHD.toLowerCase() == 'false') {
		var parent = $('#grid_ds_sanpham_nhd-pager_left');
		parent.find('.ui-icon.ui-icon-print').parent().parent().remove();
	}

	createRightPanel('grid_ds_sanpham_nhd');
	createRightPanel('grid_ds_quycachdonggoi_nhd');
	createRightPanel('grid_ds_giafob_nhd');
	createRightPanel('grid_ds_gianoi_nhd');
	createRightPanel('grid_ds_khachhangdocquyen_nhd');
	createRightPanel('grid_ds_cangxuathang_nhd');
	createRightPanel('grid_ds_bientau_nhd');
    createRightPanel('grid_ds_nhacungung_nhd');
    createRightPanel('grid_hanghoanhd_danhmuchanghoa');
	$('#tab-product-details_nhd').tabs();
</script>