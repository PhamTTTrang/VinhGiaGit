...
public partial class PrintControllers_InNhomNangLuc_Default : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
		...
		select
			cl.code_cl as [Hệ Hàng], nnl.nhom as [Nhóm],
			nnl.mota_tiengviet as [Mô Tả Tiếng Việt], nnl.hscode as [HS Code], 
			nnl.thoigianlamhang as [Thời gian làm hàng], nnl.mota as [Ghi chú]
		from md_nhomnangluc nnl , md_chungloai cl 
		where nnl.md_chungloai_id = cl.md_chungloai_id {0}", filter);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            s1.CreateRow(0).CreateCell(0).SetCellValue("NHÓM NĂNG LỰC");
            ...
            foreach (DataColumn col in dt.Columns)
            {
				...
				if("Mô Tả Tiếng Việt,Thời gian làm hàng".Contains(col.ColumnName)) {
					s1.SetColumnWidth(i, 6000);
				}
				else if(col.ColumnName == "Ghi chú") {
            ...
            String saveAsFileName = "NhomNangLuc.xls";
