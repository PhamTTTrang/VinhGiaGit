...
public partial class PrintControllers_InDMDG_Default : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
		...
		select
			dg.ma_donggoi as [Mã đóng gói], dg.ten_donggoi as [Tên đóng gói], dg.sl_inner as [SLInner], 
			(select dvt.ten_dvt from md_donvitinh dvt where dvt.md_donvitinh_id = dg.dvt_inner) as [DVTInner], 
			dg.l1 as [L1],dg.w1 as [W1],dg.h1 as [H1], dg.sl_outer as [SLOuter], 
			(select dvt.ten_dvt from md_donvitinh dvt where dvt.md_donvitinh_id = dg.dvt_outer) as [DVTOuter], 
			dg.l2_mix as [L2], dg.w2_mix as [W2], dg.h2_mix as [H2],dg.v2 as [V2],dg.vd as [VD],dg.vn as [VN],
			dg.vl as [VL],dg.ghichu_vachngan as [Ghi chú],dg.ngayxacnhan as [Ngày xác nhận],
			dg.soluonggoi_ctn as [SL Gói/cnt40]
		from md_donggoi dg where 1=1 {0}", filter);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            {   
				...
				foreach (DataColumn col in dt.Columns)
				{
					...
					string[] arr_number = {
						"SLInner","SLOuter","L1","W1","H1","L2",
						"W2","H2","V2","SL Gói/cnt40", "VD","VN","VL"
					};
