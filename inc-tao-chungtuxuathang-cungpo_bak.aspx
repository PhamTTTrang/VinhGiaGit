...
<div class="ui-layout-south ui-widget-content" id="layout-south-chungtuxuathang-cungpo">    
    ...
    <div class="ui-layout-center ui-widget-content" id="layout-center-phieuxuat-cungpo">
        ...
        <div class="ui-layout-south ui-widget-content" id="layout-south-soanthao-cungpo">
            ...
            ColNames="['c_dongnhapxuat_id', 'Nhập Xuất', 'Chi Tiết Đơn Hàng', 'STT', 'Mã SP', 'Mã SP', 'Mô Tả TV', 'Đơn Vị Tính', 'Số Lượng Phải Nhập/Xuất', 'Số Lượng Thực Nhập/Xuất', 'Đơn Giá', 'Số Kiện Thực Tế', 'Ngày Tạo', 'Người Tạo', 'Ngày Cập Nhật', 'Người Cập Nhật', 'Mô tả', 'Hoạt động']"
            RowNumbers="true"
            ColModel = "[
            {
...
<script>
    ...
    $(document).ready(function() {
                ...
                [
                    {
                        ...
                        $("#create_pkl_partner").ajaxStart(function() {
                            ...
                            var form = "#frmFromKH";
                            data1["txtSoPackingList"] = $(form + " #txtKHSoPackingList").val();
                            data1["txtSoInvoice"] = $(form + " #txtKHSoInvoice").val();
                            data1["txtNgayLap"] = $(form + " #txtKHNgayLap").val();
                            data1["txtSoTau"] = $(form + " #txtKHSoTau").val();
                            //data1["selNoiDi"] = $(form + " #selKHNoiDi").val();
                            data1["txtNoiDen"] = $(form + " #txtKHNoiDen").val();
                            data1["txtSoVanDon"] = $(form + " #txtKHSoVanDon").val();
                            data1["txtNgayVanDon"] = $(form + " #txtKHNgayVanDon").val();
                            data1["txtHangHoa"] = $(form + " #txtKHHangHoa").val();
                            data1["txtHangHoaVn"] = $(form + " #txtKHHangHoaVn").val();
                            data1["txtDienGiaiCongThem"] = $(form + " #txtKHDienGiaiCongThem").val();
                            data1["txtGiaTriCongThem"] = $(form + " #txtKHGiaTriCongThem").val();
                            data1["txtDienGiaiTruLai"] = $(form + " #txtKHDienGiaiTruLai").val();
                            data1["txtGiaTriTruLai"] = $(form + " #txtKHGiaTriTruLai").val();
                            //
							data1["txtTongGiaTriTruPO"] = $(form + " #txtKHTongGiaTriTruPO_po").val();
                            data1["txtGiaTriTruPO"] = $(form + " #txtKHGiaTriTruPO_po").val();
							//
							data1["txtTongGiaTriCongPO"] = $(form + " #txtKHTongGiaTriCongPO_po").val();
                            data1["txtGiaTriCongPO"] = $(form + " #txtKHGiaTriCongPO_po").val();
							try{
								data1["arrayPhieuXuat"] = $('#grid_ds_xuatkho_cungpo').getGridParam('selarrrow').join(", ");
							}catch(r){
								data1["arrayPhieuXuat"] = $('#grid_ds_xuatkho_cungpo').getGridParam('selarrrow');
							}
                            $.ajax({
                                url: "Controllers/TaoPackingListController.ashx?action=add",
                                type: "POST",
                                data: { p: data1 },
                                error: function(rs) {
                                ...
                                success: function(rs) {
									...
									$('#txtKHTongGiaTriCongPO_po').val(-1);
									$('#txtKHTongGiaTriTruPO_po').val(-1);
									$('#txtKHGiaTriCongPO_po').val(0);
									$('#txtKHGiaTriTruPO_po').val(0);
                                }
                            });
		...
		//-- Tinh lai phi cong tru PO
		$('#btn_load_from_po').click(function() {
			var data1 = new Object();
			var form = "#frmFromKH";
			//
			data1["txtTongGiaTriTruPO"] = $(form + " #txtKHTongGiaTriTruPO_po").val();
			...
			$.ajax({
				url: "Controllers/TaoPackingListController.ashx?action=load",
				type: "POST",
				data: { p: data1 },
				error: function(rs) {
					Popup("Error", 450, 300, rs.responseText);
				...
				success: function(rs) {
					rs = rs.split(',');
					$('#txtKHTongGiaTriCongPO_po').val(rs[0]);
					$('#txtKHTongGiaTriTruPO_po').val(rs[1]);
				}
			});
		});
	});
    ...
    <div style="width:990px; margin:auto">
            <table id="frmFromPO" class="frm-packinglist" style="width:100%; text-align:left">
                <tr>
                    <td>Số Packing List/Invoice</td>
                    <td>:</td>
                    <td><input id="txtSoPackingList" name="txtSoPackingList" type="text" /></td>
                    <%--<td>Số Invoice</td>
                    <td>:</td>--%>
                    <td><input id="txtSoInvoice" name="txtSoInvoice" type="hidden" /></td>
                </tr>
                ...
                <tr>
                    ...
                    <td>Số Tàu(M/V)</td>
                    <td>:</td>
                    <td><input id="txtSoTau" name="txtSoTau" type="text" /></td>
                </tr>
                ...
                <tr>
                    ...
                    <td>:</td>
                    <td>
                         <input id="txtNoiDen" name="txtNoiDen" />
                    </td>
                </tr>
                ...
                <tr>
                    <td>Diễn Giải Cộng (INV)</td>
                    <td>:</td>
                    <td><input id="txtDienGiaiCongThem" name="txtDienGiaiCongThem" type="text" /></td>
                    <td>Giá Trị Cộng (INV)</td>
                    <td>:</td>
                    <td><input id="txtGiaTriCongThem" name="txtGiaTriCongThem" type="text" value="0" /></td>
                </tr>
                
                <tr>
                    <td>Diễn Giải Trừ (INV)</td>
                    <td>:</td>
                    <td><input id="txtDienGiaiTruLai" name="txtDienGiaiTruLai" type="text" /></td>
                    <td>Giá Trị Trừ (INV)</td>
                    <td>:</td>
                    <td><input id="txtGiaTriTruLai" name="txtGiaTriTruLai" type="text" value="0" /></td>
                </tr>
                
				<tr>
                    <td>Tổng giá trị cộng PO</td>
                    <td>:</td>
                    <td><input id="txtKHTongGiaTriCongPO_po" name="txtKHTongGiaTriCongPO_po" type="text" value="-1" readonly /></td>
					<td>Giá Trị cộng (P/O) cần điều chỉnh (INV)</td>
                    <td>:</td>
                    <td><input id="txtKHGiaTriCongPO_po" name="txtKHGiaTriCongPO_po" type="text" value="0" /></td>
                    <td></td>
                    <td></td>
                </tr>
				
				<tr>
                    <td>Tổng giá trị trừ PO</td>
                    <td>:</td>
                    <td><input id="txtKHTongGiaTriTruPO_po" name="txtKHTongGiaTriTruPO_po" type="text" value="-1" readonly /></td>
                    <td>Giá Trị trừ (P/O) cần điều chỉnh (INV)</td>
                    <td>:</td>
                    <td><input id="txtKHGiaTriTruPO_po" name="txtKHGiaTriTruPO_po" type="text" value="0" /></td>
                    <td></td>
                    <td></td>
                </tr>
                ...
                <tr>
                    ...
                    <td rowspan="5" class="submit">
						<button id="btn_load_from_po">Tính lại phí cộng trừ PO</button>
